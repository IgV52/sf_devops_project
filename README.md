...
```
git clone https://github.com/IgV52/sf_devops_project.git
```
...
3.2) Заходим в директорию `./terraform/main.tf/`, далее файл `.key.json` необходимо поместить в данную директорию. Этот файт требуется для подключения Terraform к сервисному аккаунту и выполнения задач. Инструкция - https://yandex.cloud/ru/docs/iam/operations/sa/create-access-key#cli_2

3.3) В файле `./terraform/modules/instance_module.tf` в блоке `metadata` реализована отправка публичного ssh-ключа на все три ВМ. При необходимости, можно поменять файл на другой публичный ssh-ключ.
...
sudo kubeadm join <app ip:port> --token <токен_из_первой_команды> --discovery-token-ca-cert-hash sha256:<токен_из_второй_команды>
```
